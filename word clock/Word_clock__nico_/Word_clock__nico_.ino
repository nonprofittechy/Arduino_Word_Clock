...
ledWord HAPPY[]   =   { {2,8+4+2+1},
                        {2,128}};
ledWord PAST[]    =   { {0,0},
                        {2, 127}}; // MAGIC VALUE -- SOMETHING IS ODD IN WIRING
...
bool isBirthday(tmElements_t tm) {
  if ((tm.Month == BMONTH) && (tm.Day == BDAY)) {
    return true;
  } else {
    return false;
  }
...
void lightMinuteWords(int mn) {
  if (mn < 5) {
    // nothing to do, we set the "dots" elsewhere
  } else if (mn < 10) {
    // IT IS FIVE PAST ONE
    //FIVE
...
/*
 * test Lighting up the right words for the current hour, in 12 hour format
 */
void testHours() {
